...
<?php
    ...
    if(isset($_POST["nom"]) && !empty($_POST["nom"]) && 
        ...
        $valid=grainDeSel(rand(5,20));

        //Mes données sont saines, je peux les envoyer dans la db
        sendMail($email,$valid);
        createUser($photo, $login, $nom, $prenom, $email, $mdpToSend, $role, $roleId, $valid);
?>
        <!-- Si tout c'est bien passé, informer et proposer de se connecter -->
        <h2 class="registerOk">Un code de validation vous à été envoyé par mail, entrez ce code pour pouvoir utilisé pleinement votre compte, connectez vous et utilisez le code de validation <a href="../../src/pages/login.php"> Se connecter</a> </h2>
